...
/**
 * Abstract type serving as a DI token for the service converting from your
 * application Date model to internal NgbDateStruct model. A default
 * implementation converting from and to NgbDateStruct is provided for
 * retro-compatibility, but you can provide another implementation to use an
 * alternative format, ie for using with native Date Object.
 */
@Injectable()
export abstract class NgbDateAdapter<T> {
  /**
   * Converts user-model date into an NgbDateStruct for internal use in the
   * library
   ...
   * The returned type is suposed to be of the same type as fromModel()
   * input-value param
